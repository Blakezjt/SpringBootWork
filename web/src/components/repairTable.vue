<template>
    <div class="demo-input-size">
      ...
      <div>学号：<el-input v-model="input2" class="inputStyle" placeholder="Please Input" /><el-button type="primary"
          :icon="Search" >Search</el-button></div>
    </div>
    ...
    <el-table :data="tableList" stripe style="width: 100%" >
      ...
      <el-button type="primary" :icon="Edit" circle @click="dialogFormVisible = true" :disabled="isDisabled"/>
      <el-button type="danger" :icon="Delete" circle :disabled="isDisabled"/>
    </el-table-column>
    </el-table>
  ...
  import { findAllRepairByPages } from '../api/repairTable.js'
  ...
  const AllRepairByPages = () => {
    const url = ref('')
    if(route.fullPath == '/manMain/repairTable'){
      url.value = '/repairs/PluRepairsList';
    }  else if(route.fullPath == '/pluMain/repairTable'){
      url.value = '/repairs/PluRepairsList';
      isDisabled.value = false;
    }
    let result = findAllRepairByPages(url.value,currentPage4.value, pageSize4.value);
